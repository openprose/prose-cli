...
The ephemeral `--service-environment` option applies to these commands. Read
...
only the receipt, not local paths. Human staging output includes an explicit
stderr indicator; structured output has no banner. Publishing and fetching do not
start a harness.

Invalid invocation and local configuration failures exit with code 2. Service
failures exit with code 10. HTTP 401/403 require authentication; 400/404/429 and
unavailable service responses report `SERVICE_UNAVAILABLE`; 409 reports
